...
class ShareLimiterInterface(object):
    ...
    def submit(self, connection_ref, job_id, current_difficulty, timestamp, worker_name):
        ...
        connection_ref().rpc('mining.set_difficulty', [settings.POOL_TARGET,], is_notification=True)
...
class ShareManagerInterface(object):
    ...
    def on_submit_share(self, worker_name, block_header, block_hash, difficulty, timestamp, is_valid, ip, invalid_reason, share_diff):
        log.info("%s %s %s %s %s %s %s %s %s %s" %(worker_name, block_hash, difficulty, timestamp, 'valid' if is_valid else 'INVALID', ip, self.block_height, self.prev_hash,
    ...
    def on_submit_block(self, on_submit, worker_name, block_header, block_hash, difficulty, timestamp, ip, share_diff, prev_height, prev_block_hash):
        is_accepted = on_submit

        log.info("%s %s %s %s %s %s %s %s %s" %(worker_name, block_hash, difficulty, timestamp, 'ACCEPTED' if is_accepted else 'REJECTED', ip, 
                prev_height, prev_block_hash, share_diff))
        dbi.found_block([worker_name, block_header, block_hash, difficulty, timestamp, is_accepted, ip, prev_height, prev_block_hash, share_diff ])
