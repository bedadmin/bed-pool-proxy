...
class MiningService(GenericService):
            ...
            for subscription in Pubsub.iterate_subscribers(self.event):
                try:
                    if subscription and subscription.connection_ref():
                        subscription.connection_ref().rpc('mining.set_difficulty', [diff], is_notification=True)
                except:
                    pass
            return True
        return False
    ...
    def configure(self, extensions, parameters):
        #{"error": null, "id": 1, "result": {"version-rolling": false}}
        return {
            "version-rolling": True,
            "version-rolling.mask": "20000000"
        }
    ...
    def subscribe(self, miner_version, *args):
        ...
        session = self.connection_ref().get_session()
        session['miner_version'] = miner_version
    ...
    def submit(self, worker_name, work_id, extranonce2, ntime, nonce, version_rolling=None):
        ...
        extranonce1 = binascii.hexlify(extranonce1_bin)
        ...
        prev_height, prev_block_hash = (Interfaces.share_manager.block_height, Interfaces.share_manager.prev_hash)
        ...
        try:
            (block_header, block_hash, share_diff, on_submit) = Interfaces.template_registry.submit_share(job_id,
                worker_name, session, extranonce1_bin, extranonce2, ntime, nonce, difficulty, ip, version_rolling)
            ...
            log.info("%s-%s:%s (%d, %d, %s, %s, %d) %0.2f%% work_id(%s) job_id(%s) diff(%f)" % (self.connection_ref().get_ident(), worker_name, (extranonce1, extranonce2, nonce, ntime, version_rolling), valid, invalid, is_banned, is_ext_diff, last_ts, percent, work_id, job_id, difficulty))
            if is_banned:
                raise SubmitException("Worker is temporarily banned")

            Interfaces.share_manager.on_submit_share(worker_name, '', '', difficulty, submit_time, False, ip, e[0], 0)
            raise
        ...
        if on_submit != None:
            ...
            on_submit.addCallback(Interfaces.share_manager.on_submit_block,
                worker_name, block_header, block_hash, difficulty, submit_time, ip, share_diff, prev_height, prev_block_hash)
        else:
