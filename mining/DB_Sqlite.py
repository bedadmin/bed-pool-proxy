...
#        data: [worker_name,block_header,block_hash,difficulty,timestamp,is_valid,ip,block_height,prev_hash,invalid_reason,share_diff]
        ...
        if settings.DATABASE_EXTEND :
            ...
            round_shares = float(self.dbc.fetchone()[0]) + total_shares
            ...
            round_best_share = float(self.dbc.fetchone()[0])
    ...
    def found_block(self, v):
        log.debug("Importing Accepted Shares")
        data = {'time':v[4],'rem_host':v[6],'username':v[0],'our_result':True,'upstream_result':v[5],'reason':'','solution':v[2],
                                'block_num':v[7],'prev_block_hash':v[8],'ua':'','diff':v[3]}
        self.dbc.execute("insert into shares " +\
                "(time,rem_host,username,our_result,upstream_result,reason,solution,block_num,prev_block_hash,useragent,difficulty) " +\
                "VALUES (:time,:rem_host,:username,:our_result,:upstream_result,:reason,:solution,:block_num,:prev_block_hash,:ua,:diff)", data)
